...
app.get('/set_cookie', function (request, response) {
    ...
    var my_name = "Lexy";
    response.cookie("your_name", my_name).send('cookie set'); //sets cookie to be name = my_name
});
...
app.get("/login", function (request, response) { //get to login page
    console.log(request.cookies); // print out query string
...
app.post("/check_login", function (request, response) {
    ...
    if (typeof userdata[login_username] != 'undefined') {
        ...
        if (user_info.password != request.body["password"]) {//check if passowrd stored for username matches what user entered
            ...
            response.cookie("username", login_username); //sets cookie on login
            response.end(`${login_username} is logged in with data ${JSON.stringify(quantity_str)} on ${theDate}`); //change this to response.redirect for assignment 2
            return; 
        } 
} else {
